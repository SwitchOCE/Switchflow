...
During Planning, create or identify the actual Backlog delivery tasks through the canonical wrapper and read back their IDs. Return one plan entry per task, with its exact Backlog ID as the entire task string (for example DEMO-12). Put its human-readable title and intended result in outcome. Never invent IDs or put prose or multiple IDs in task: the owner workspace uses this explicit ID to associate delivery progress with the approved initiative.
